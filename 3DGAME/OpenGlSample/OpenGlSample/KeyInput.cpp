...
void KeyInput::computeMatricesFromInputs()
{
	...
	// Projection matrix : 45воиб Field of View, 4:3 ratio, display range : 0.1 unit <-> 100 units
...
void KeyInput::computeMatricesFromInputs_WASD()
{
	// glfwGetTime is called only once, the first time this function is called
	static double lastTime = glfwGetTime();
	...
	// Projection matrix : 45воиб Field of View, 4:3 ratio, display range : 0.1 unit <-> 100 units
	ProjectionMatrix_WASD = glm::perspective(glm::radians(FoV), 4.0f / 3.0f, 0.1f, 100.0f);
	// Camera matrix
	ViewMatrix_WASD = glm::lookAt(
		position,           // Camera is here
		position + direction, // and looks here : at the same position, plus "direction"
		up                  // Head is up (set to 0,-1,0 to look upside-down)
	);
